...
public class AddRoomDialog extends JDialog {
    ...
    private Map<Integer, String> hostelMap;   // hostel_id -> hostel_name (type)
    private boolean confirmed = false;

    public AddRoomDialog(JFrame parent) {
        super(parent, "Add New Room", true);
        RoomDAO roomDAO = new RoomDAO();
        // For hostel selection, we'll use a map of hostel_id -> "hostel_name (type)"
        // We'll fetch hostels separately; roomDAO doesn't have a direct method, so we add a helper.
        hostelMap = new java.util.LinkedHashMap<>();
        // Quick inline query (you can also add a method to RoomDAO to fetch hostels)
